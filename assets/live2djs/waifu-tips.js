...
const live2d_settings = {
  // 基本设置
  modelUrl: "/assets/models", // 存放模型的文件夹路径，末尾不需要斜杠
  tipsMessage: "/assets/live2djs/waifu-tips.json", // 看板娘提示消息文件的路径，可以留空不加载
  // 模型设置
  modelName: "黍", // 默认加载的模型名称，仅在无本地记录的情况下有效
...
function initModel() {
  ...
  if (getSS("waifuHide") === "1") {
    ...
    window.innerWidth <= Number(live2d_settings.waifuMinWidth.replace("px", ""))
  ) {
    waifu.classList.add("hide");
    return;
  } else {
  ...
  if (!live2d_settings.showVolumeBtn)
    $$(".waifu-tool .icon-volumeup").classList.add("hide") ||
      $$(".waifu-tool .icon-volumedown").classList.add("hide");
  $$(".waifu-tool .icon-next").addEventListener("click", () =>
    loadOtherModel()
  );
  $$(".waifu-tool .icon-home").addEventListener(
    "click",
  ...
  window.live2dv2.debugMousemove =
    live2d_settings.debug && live2d_settings.debugMousemove;
  window.live2dv4.debugMousemove =
    live2d_settings.debug && live2d_settings.debugMousemove;
    ...
    testWebP()
      ...
      .then(() => {
        if (window.webpReady === true)
          console.log(
            "[WaifuTips] Your browser support WebP format. Try to load WebP texture first."
          );
        else
...
function loadModel(modelName) {
  ...
  else setSS("modelName", modelName);
  ...
  if (modelVersion === 2) {
    ...
    window.live2dv2.load(
      live2dId2,
      `${live2d_settings.modelUrl}/${modelName}/model.json`
    );
...
function loadOtherModel() {
  ...
  if (live2d_settings.modelRandMode) {
    ...
    modelIndex = live2d_models.findIndex(
      (modelObj) => modelObj.name === modelName
    );
    if (modelIndex < live2d_models.length - 1) modelIndex++;
    else modelIndex = 0;
  }
...
function loadTipsMessage(result) {
  ...
  const addMouseoverListener = () => {
    for (let tips of result.mouseover) {
      ...
      else if (select) mouseenterListener(select, tips);
      else
        live2d_settings.debug &&
          console.warn(`[WaifuTips] can not found element: ${tips.selector}`);
    }
  };
  ...
  const addCopyListener = () => {
    if ($$("#articleContent").length !== 0)
      $$("#articleContent").addEventListener("copy", () =>
  ...
  function showHitokoto() {
    switch (live2d_settings.hitokotoAPI) {
      ...
      case "fghrsh.net":
        window
          .fetch("https://api.fghrsh.net/hitokoto/rand/?encode=jsc&uid=3335")
          .then((res) => res.json())
          .then((resJson) => {
            if (!resJson.source) {
