...
// 模型列表
// SDKv5兼容2 3 4版本模型，因此实际上不需要Version信息，为保持兼容，保留该信息。
const live2d_models = [
    {
...
function initModel() {
    ...
    // 初始化WebGL
    const canvas = document.getElementById(live2dId2);
    ...
    $$(`#${live2dId5}`).setAttribute('height', live2d_settings.live2dHeight);
    $$(`#${live2dId5}`).setAttribute('width', live2d_settings.live2dWidth);
    ...
    if (!live2d_settings.showVolumeBtn) $$('.waifu-tool .icon-volume').classList.add('hide');
    ...
    $$('.waifu-tool .icon-camera').addEventListener('click', () => {
        window.live2dCurrentVersion === 3 ? window.live2dv5.CaptureCanvas() : window.live2dv2.captureFrame = true;
    });

    $$('.waifu-tool .icon-cross').addEventListener('click', () => {
        sessionStorage.setItem('waifuHide', '1');
        showMessage('愿你有一天能与重要的人重逢。', 1500, true);
        window.setTimeout(() => {
            waifu.classList.add('hide');
        }, 1500);
    });
    $$('.waifu-tool .icon-next').addEventListener('click', () => {
        loadOtherModel();
    });
    $$('.waifu-tool .icon-message').addEventListener('click', () => {
        showHitokoto();
    });
    $$('.waifu-tool .icon-home').addEventListener('click', () => {
        window.location.href = live2d_settings.homePageUrl;
    });
    $$('.waifu-tool .icon-about').addEventListener('click', () => {
        window.open(live2d_settings.aboutPageUrl);
    });
    $$('.waifu-tool .icon-volume').addEventListener('click', () => {
        const t = $$('#live2d-volume');
    ...
    // 定义SDK初始化函数
    const initSdk = () => {
        window.live2dv5.setPreLoadMotion(live2d_settings.preLoadMotion);
        window.live2dv5.debug = live2d_settings.debug;
        window.live2dv5.debugMousemove = live2d_settings.debug && live2d_settings.debugMousemove;
        // 初始化模型
        let modelName = modelStorageGetItem('modelName');
        if (modelName === null) modelName = live2d_settings.modelName;
        if (live2d_settings.tryWebp) {
            testWebP().then(supportWebP => {
                if (supportWebP)
                    console.log("[WaifuTips] Your browser support WebP format.");
                else
...
function loadModel(modelName) {
    ...
    if (window.live2dCurrentVersion !== modelVersion) {
        if (window.live2dCurrentVersion === 2) {
            ...
            window.live2dv5.release();
            $$(`#${live2dId5}`).style.display = 'none';
        }
    }
    // 根据模型版本选择不同的SDK加载
    // 若未加载任何模型
    // 确保模型路径格式正确
    const modelPath = `${live2d_settings.modelUrl}/${modelName}`;
    if(modelVersion==2){
        $$(`#${live2dId5}`).style.display = 'none';
        $$(`#${live2dId2}`).style.display = 'block';
        window.live2dv2.load(live2dId2, `${live2d_settings.modelUrl}/${modelName}/model.json`);
    } else {
        $$(`#${live2dId5}`).style.display = 'block';
        $$(`#${live2dId2}`).style.display = 'none'; 
        window.live2dv5.load(live2dId5, modelPath, `${modelName}.model3.json`);
    }
