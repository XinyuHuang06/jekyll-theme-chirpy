...
export class LAppView {
  ...
  public initializeSprite(): void {
    ...
    // 背景画像初期化
    const backImageName = LAppDefine.BackImageName;
    // 只有当背景图像名称不为null时才加载背景图像
    if (backImageName) {
      // 非同期なのでコールバック関数を作成
      ...
      // 如果没有指定背景图像，将_back设置为null
      this._back = null;
      if (LAppDefine.DebugLogEnable) {
        console.log("[APP] Background image disabled");
      }
    }

    // 歯車画像初期化
    const gearImageName = LAppDefine.GearImageName;
    // 只有当齿轮图像名称不为null时才加载齿轮图像
    if (gearImageName) {
      ...
      textureManager.createTextureFromPngFile(
        resourcesPath + gearImageName,
        false,
        initGearTexture
      );
    } else {
      // 如果没有指定齿轮图像，将_gear设置为null
      this._gear = null;
      if (LAppDefine.DebugLogEnable) {
        console.log("[APP] Gear image disabled");
      }
    }
