...
export class LAppModel extends CubismUserModel {
  ...
  public loadAssets(dir: string, fileName: string): void {
    ...
    // 确保目录路径以斜杠结尾
    const modelDir = dir.endsWith('/') ? dir : dir + '/';

    fetch(`${modelDir}${fileName}`)
      ...
      .catch(error => {
        ...
        CubismLogError(`Failed to load file ${modelDir}${fileName}`);
      });
  }
  ...
  public getHitAreaNames(): string[] {
    ...
    for (let i = 0; i < count; i++) {
      names.push(this._modelSetting.getHitAreaName(i));
    }
   ...
   * @param group モーショングループ名
   * @param index グループ内の識別番号
   * @return 読み込まれたモーションのインスタンス
   */
  public loadMotion(
    buffer: ArrayBuffer,
    size: number,
    ...
    if (motion) {
      // 循环参数从motion3.json文件加载，但我们希望默认关闭循环
      // 因为 "黍" 模型的动作都设置了循环，这会导致动作不会结束
  ...
  /**
   * 模型重置到初始状态
   * 通过加载保存的初始参数，使模型回到默认姿态
   * @param fadeInSeconds 过渡到初始状态的时间（秒）
   */
  public resetToInitialState(fadeInSeconds: number = 0.5): void {
    if (!this._model) {
      return;
    }
    ...
    // 检查是否有Idle动作组
    if (this._modelSetting.getMotionCount('Idle') > 0) {
      // 如果有Idle动作组，使用其中的动作
      this.startRandomMotion('Idle', LAppDefine.PriorityIdle);
    } else {
      // 查看是否有其他可用的动作组作为默认状态
      for (let i = 0; i < this._modelSetting.getMotionGroupCount(); i++) {
        const groupName = this._modelSetting.getMotionGroupName(i);
        if (groupName && this._modelSetting.getMotionCount(groupName) > 0) {
          // 使用找到的第一个动作组的第一个动作
          this.startMotion(groupName, 0, LAppDefine.PriorityIdle);
          break;
        }
      }
    }
    ...
    if (this._debugMode) {
      LAppPal.printMessage('[APP]Model reset to initial state');
    }
  }
}
