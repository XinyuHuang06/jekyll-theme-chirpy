...
{\left( \boldsymbol{x}_r-\boldsymbol{c}_r \right)}^H \chi_{n,r} {\left( \boldsymbol{x}_r-\boldsymbol{c}_r \right)}- \vartheta-h=0 \quad n\in [1,2,\cdots,N] \\
...
$$
\begin{array}{l}
\mathcal{L}\left( \boldsymbol{s}_r,\boldsymbol{b}_r,\boldsymbol{u},\left\{v_n \right\}_{n=1}^N,h \right)=
...
&+\sum_{n=1}^{N} v_n \left({\left( \boldsymbol{b}_r-\boldsymbol{c}_r \right)}^H \chi_{n,r} {\left( \boldsymbol{x}_r-\boldsymbol{c}_r \right)}- \vartheta-h\right)\\
&+\sum_{n=1}^{N} \frac{\rho_n}{2} {\Vert{\left( \boldsymbol{b}_r-\boldsymbol{c}_r \right)}^H \chi_{n,r} {\left( \boldsymbol{x}_r-\boldsymbol{c}_r \right)}- \vartheta-h\Vert}^2
\end{array}
\label{larfun_1}
$$

其中，$\left\{ \rho_n \vert n=0,1,\cdots,N \right\}$为先验参数，用于控制算法收敛的进度。
## 算法详解

采用ADMM算法求解式$\eqref{larfun_1}$。

- Step 1：求解$\boldsymbol{s}_r^{k+1}$

  确定$\boldsymbol{b}_r^{k},\boldsymbol{u}^{k},\left\{v_n^k \right\}_{n=1}^N,h^k$。此时仅仅只有$\boldsymbol{s}_r$变量，此时目标函数如下：
  $$
  \begin{array}{ll}
  
  \min\limits_{\boldsymbol{s}_r} & \mathcal{L}\left( \boldsymbol{s}_r,\boldsymbol{b}_r,\boldsymbol{u},\left\{v_n \right\}_{n=1}^N,h \right)  \\
  s.t. & \boldsymbol{s}_r^T\boldsymbol{s}_r=1
  \end{array}
  $$
  ...
  \left[
  \boldsymbol{x}_{r}^T \boldsymbol{F}_{N,r}^T \varGamma_{\alpha,f}^{1,T} \left( \boldsymbol{F}_{N,r} \boldsymbol{b}_r \right)
  {\left( \boldsymbol{F}_{N,r}\boldsymbol{b}_r \right)}^T \varGamma_{\alpha,f}^1 {\boldsymbol{F}_{N,r}\boldsymbol{x}_r}
  + \right. \\
  &
  \left[
  \boldsymbol{x}_{r}^T \boldsymbol{F}_{N,r}^T \varGamma_{\alpha,f}^{2,T} \left( \boldsymbol{F}_{N,r} \boldsymbol{b}_r \right)
  {\left( \boldsymbol{F}_{N,r}\boldsymbol{b}_r \right)}^T \varGamma_{\alpha,f}^2 {\left( \boldsymbol{F}_{N,r}\boldsymbol{x}_r \right)}\right]+
...
- Step 4：求解$\boldsymbol{u},\left\{v_n \right\}_{n=1}^N$
