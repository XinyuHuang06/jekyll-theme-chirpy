...
- Step 1：求解$\boldsymbol{x}_r^{k+1}$
  ...
  \min\limits_{\boldsymbol{x}_r} & \mathcal{L}\left( \boldsymbol{x}_r\right)  \\
  ...
  $$
  \mathcal{L}\left( \boldsymbol{x}_r\right)=\boldsymbol{x}_r^T 		 
  \boldsymbol{A}\boldsymbol{x}_r+\boldsymbol{B}\boldsymbol{x}_r+\boldsymbol{C}
  $$
  ...
  \boldsymbol{B}=
  &
  \boldsymbol{u}^T-\rho_0\boldsymbol{b}_r+\sum\limits_{n=1}^{N}v_n \chi_{n,r} \left( \boldsymbol{b}_r-\boldsymbol{c}_r \right) \\
  &+
  \end{array}
  $$
